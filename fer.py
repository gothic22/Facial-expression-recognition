...
from preprocess_fer2013 import num_train,num_private,num_public
...
class FER2013(data.Dataset):
    ...
    def __init__(self, split='Training', transform=None):
        ...
        if self.split == 'Training':
            ...
            self.train_data = self.train_data.reshape((num_train, size1, size1))
            ...
            self.PublicTest_data = self.PublicTest_data.reshape((num_public, size1, size1))

        else:
            self.PrivateTest_data = self.data['PrivateTest_pixel']
            self.PrivateTest_labels = self.data['PrivateTest_label']
            self.PrivateTest_data = np.asarray(self.PrivateTest_data)
            self.PrivateTest_data = self.PrivateTest_data.reshape((num_private, size1, size1))
